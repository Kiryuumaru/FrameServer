...
public static class CommonConfigurationExtensions
{
    ...
    public static Guid GetRuntimeGuid(this IConfiguration configuration)
    {
        ...
        {
            _runtimeGuid = Guid.Parse(configuration.GetVarRefValueOrDefault($"{ApplicationDefaults.EnvironmentVarsPreName}_RUNTIME_GUID", Guid.NewGuid().ToString()));
        }
        return _runtimeGuid.Value;
    }

    public const string MakeFileLogsKey = $"{ApplicationDefaults.EnvironmentVarsPreName}_MAKE_LOGS";
    public static bool GetMakeFileLogs(this IConfiguration configuration)
    {
        return configuration.GetVarRefValueOrDefault(MakeFileLogsKey, "no").Equals("yes", StringComparison.InvariantCultureIgnoreCase);
    }
    public static void SetMakeFileLogs(this IConfiguration configuration, bool makeFileLogs)
    {
        configuration[MakeFileLogsKey] = makeFileLogs ? "yes" : "no";
    }

    public const string LoggerLevelKey = $"{ApplicationDefaults.EnvironmentVarsPreName}_LOGGER_LEVEL";
    ...
    public const string HomePathKey = $"{ApplicationDefaults.EnvironmentVarsPreName}_HOME_PATH";
